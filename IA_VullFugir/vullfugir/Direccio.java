...
public enum Direccio {
	N(0, -1, -1), NE(1, 0, -1), SE(1, 1, 0), S(0, 1, 1), SW(-1, 1, 0), NW(-1,
			0, -1), C(0, 0, 0);
	...
	public int incColumna() {
		return incColumna;
	}

	public int incFilaParell() {
		return incFilaParell;
	}

	public int incFilaSenar() {
		return incFilaSenar;
	}
	...
	public Punt coordenadesVeinat(Punt coord) {
		...
		int fila = coord.y
				+ (Util.isEven(coord.x) ? incFilaParell() : incFilaSenar());
